...
public class ManagerDetail extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			String managerbrandno = request.getParameter("managerbrandno");
			int managerproductno = Integer.parseInt(request.getParameter("managerproductno"));

			conn = ds.getConnection();
			pstmt = conn.prepareStatement("select * from profinal where brand_no=? and product_no=?");
			pstmt.setString(1, managerbrandno);
			pstmt.setInt(2, managerproductno);		
		...
		RequestDispatcher rd = getServletContext().getRequestDispatcher("/ManagerDetail.jsp");
